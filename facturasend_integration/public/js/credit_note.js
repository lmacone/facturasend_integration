...
// Similar functionality for Credit Notes (is_return Sales Invoices)
frappe.ui.form.on('Sales Invoice', {
	refresh: function(frm) {
		if (frm.doc.docstatus === 1 && frm.doc.is_return && !frm.doc.is_debit_note) {
			...
			if (!frm.doc.facturasend_cdc || frm.doc.facturasend_estado === 'Error' || frm.doc.facturasend_estado === 'Rechazado') {
				frm.add_custom_button(__('Enviar NC a FacturaSend'), function() {
					send_credit_note_to_facturasend(frm);
				}, __('FacturaSend'));
			}
...
function send_credit_note_to_facturasend(frm) {
	frappe.confirm(
		__('¿Está seguro de enviar esta Nota de Crédito a FacturaSend?'),
		function() {
			frappe.call({
				...
				callback: function(r) {
					if (r.message && r.message.success) {
						frappe.msgprint(__('Nota de Crédito enviada exitosamente a FacturaSend'));
						frm.reload_doc();
	...
	frappe.call({
		...
		callback: function(r) {
			if (r.message && r.message.success) {
				...
				link.download = `KUDE_NC_${frm.doc.name}.pdf`;
	...
	frappe.call({
		...
		callback: function(r) {
			if (r.message && r.message.pdf_url) {
				...
				let link = document.createElement('a');
				link.href = r.message.pdf_url;
				link.download = `KUDE_NC_${timestamp}.pdf`;
				document.body.appendChild(link);
				link.click();
				document.body.removeChild(link);
